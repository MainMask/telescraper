"""Command-line entry point: `telescraper <command>`."""
...
from telescraper import __version__
...
def cmd_menu(args) -> None:
    from telescraper.menu import run_menu

    run_menu()


def cmd_read(args) -> None:
    from telescraper.datafiles import read_table, save_table
...
def cmd_links(args) -> None:
    from telescraper.analysis import links
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="telescraper", description=__doc__)
    ...
    s.add_argument("--session", default="telescraper",
                   help="session name/path (default: ./telescraper.session)")
    ...
    vf.add_argument("--session", default="telescraper",
                    help="session name/path (default: ./telescraper.session)")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if getattr(args, "func", None) is None:  # `telescraper` with no arguments
        cmd_menu(args)
        return
    args.func(args)
