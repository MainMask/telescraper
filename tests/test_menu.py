...
from telescraper.cli import build_parser, main
from telescraper.menu import Prompt, run_menu
...
def test_scrape_argv_minimal():
    from telescraper.menu import _scrape_argv
...
def test_read_argv_with_conversion(tmp_path, monkeypatch):
    ...
    from telescraper.menu import _read_argv
...
def test_comments_argv(tmp_path, monkeypatch):
    ...
    (tmp_path / "output").mkdir()
    (tmp_path / "output" / "FINAL_a.parquet").write_bytes(b"x")
    from telescraper.menu import _comments_argv
...
def test_participants_argv(tmp_path, monkeypatch):
    ...
    (tmp_path / "output").mkdir()
    (tmp_path / "output" / "FINAL_a.parquet").write_bytes(b"x")
    from telescraper.menu import _participants_argv
...
def test_menu_has_verify():
    from telescraper.menu import _ACTIONS
...
def test_cli_opens_menu(monkeypatch, argv):
    called = []
    monkeypatch.setattr("telescraper.menu.run_menu", lambda: called.append(argv))
    main(argv)
    assert called == [argv]
